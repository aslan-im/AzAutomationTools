...
task DebugBuild -if ($Configuration -eq "debug") {
    $Script:ModuleName = (Test-ModuleManifest -Path ".\src\*.psd1").Name
    ...
    if(!($ModuleVersion)) {
        ...
        $ModuleVersion = (Test-ModuleManifest -Path ".\src\$($ModuleName).psd1").Version
    ...
    try {
        Copy-Item -Path ".\src\$($ModuleName).psd1" -Destination ".\Output\temp\$($ModuleName)\$ModuleVersion\"
        New-Item -Path ".\Output\temp\$($ModuleName)\$ModuleVersion\$($ModuleName).psm1" -ItemType File
    }
    catch {
        throw "Failed copying Module Manifest from: .\src\$($ModuleName).psd1 to .\Output\temp\$($ModuleName)\$ModuleVersion\ or Generating the new psm file."
    }
    ...
    foreach($Function in $PublicFunctions.Name){
        try {
            ...
            $Content = Get-Content -Path ".\src\Public\$($Function)"
            Add-Content -Path $ModuleFile -Value "#Region - $Function"
            Add-Content -Path $ModuleFile -Value $content
            if($ExportAlias.IsPresent){
                $AliasSwitch = $false
                $Sel = Select-String -Path ".\src\Public\$($function)" -Pattern "CmdletBinding" -Context 0,1
    ...
    foreach($function in $privateFunctions.Name){
        try {
            Write-Verbose -Message "Updating the .psm1 file with function: $($function)"
            $content = Get-Content -Path ".\src\Private\$($function)"
            Add-Content -Path $ModuleFile -Value "#Region - $function"
            Add-Content -Path $ModuleFile -Value $content
            Add-Content -Path $ModuleFile -Value "#EndRegion - $function"            
        }
...
task Build -if($Configuration -eq "Release"){
    ...
    if(!($ModuleVersion)) {
        Write-Verbose -Message "No new ModuleVersion was provided, locating existing version from psd file."
        $oldModuleVersion = (Test-ModuleManifest -Path ".\src\$($ModuleName).psd1").Version
        ...
        Update-ModuleManifest -Path ".\src\$($ModuleName).psd1" -ModuleVersion $ModuleVersion
    }
    ...
    Write-Verbose -Message "Generating the Module Manifest for temp build and generating new Module File"
    try {
        Copy-Item -Path ".\src\$($ModuleName).psd1" -Destination ".\Output\$($ModuleName)\$ModuleVersion\"
        New-Item -Path ".\Output\$($ModuleName)\$ModuleVersion\$($ModuleName).psm1" -ItemType File
    }
    catch {
        throw "Failed copying Module Manifest from: .\src\$($ModuleName).psd1 to .\Output\$($ModuleName)\$ModuleVersion\ or Generating the new psm file."
    }
    ...
    Write-Verbose -Message "Removing dotsrc links"
    $ModuleFileContentWithDotsrc = Get-Content $ModuleFile
    $ModuleFileContentWithoutDotsrc = $ModuleFileContentWithDotsrc -replace '\. \$PSScriptRoot\\.*\.ps1$'
    $ModuleFileContentWithoutDotsrc | Out-File $ModuleFile
